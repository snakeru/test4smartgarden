 ...
 * SVN:                 $Rev: 2135 $ ($Date: 2020-06-07 00:27:26 +0200 (So, 07 Jun 2020) $)
 * contains some functions available at forum.iobroker.net, see function header
 */
"use strict";

// some variables 
const apirev = '$Rev: 2135 $';
...
function setGardenaObjectNotExists(s, value) {
	let arr = s.split('.');
	let loc_id;
	let dev_id;
	let serv_id;
	...
	if (arr.length === 0) {
		ju.adapterloginfo(1, 'setGardenaObjectNotExists: empty id: ' + s);
		return;
	}
	...
	if (loc.search('LOCATION_') === -1) {
		ju.adapterloginfo(1, 'setGardenaObjectNotExists: no location found: ' + s);
		return;
	} else {
								...
								switch (state.val) {
									...
									default:
										if (String(state.val).search('PAUSE') !== -1 ) {
											...
											// check warning
											let w = getSaveState(name + '.' + IRRWHILEMOWING_WARNING);
 ...
 * @param   {string}  s1    state decoded
 * @return  value of mower service if available, els UNDEFINED 
 */
function getServiceNameFromDevice(s1, service) {
	let id_offset;
	let loc;
	let dev;
	let serv;
	...
	if (arr.length <= id_offset + 0) {
		return UNDEFINED;
	}	
	loc = arr[id_offset + 0];
	
	if (arr.length <= id_offset + 1) {
		return UNDEFINED;
	}	
	dev = arr[id_offset + 1];
	...
	for (serv in LOCATIONS[loc][dev]) {
		if (serv.search(service) !== -1) {
			...
			if (service === 'SERVICE_VALVE') {
				if (serv.search('SERVICE_VALVE_SET') === -1) {
					return adapter_instance + loc + '.' + dev + '.' + serv;				
				}
			} else {
				return adapter_instance + loc + '.' + dev + '.' + serv;
			}
...
function setMower(name, a, type, propval) {
	...
	if (a === '.activity_value') {
		...
		//setGardenaObjectNotExists(nx1, val_i);
...
function setValve(name, a, type, propval) {
	...
	if (configUseIrrigationAllowedWhileMowing === true) {
		...
		if (a === '.activity_value') {
			...
			sgSaveStateNotExists(nx1, val_i);
...
function setIrrigationWarning(name, e) {
	if (getSaveState(name + '.' + IRRWHILEMOWING_WARNING) !== e) {
	...
	if (configUseIrrigationAllowedWhileMowing === true) {
		switch (type) {
			case 'VALVE':
				if (a === '.activity_value') {
					if (propval !== 'CLOSED') {
						if (getSaveState(name + '.' + IRRWHILEMOWING_ALLOWED) === false) {
							mowerDefinitionString = getSaveState(name + '.' + IRRWHILEMOWING_MOWER_DEFINITION);
							warningcode = '';
							...
							if (mowerDefinitionString.length === 0) {
								...
								for (let i = 0; i < mowerArr.length; i++) {
									...
									if (mow_i === IRRWHILEMOWING_CODE_FORBIDDEN) {
										...
										let sarr = getSaveState(mow_i + '.activity_mowing_i');
										if (sarr === UNDEFINED) {
			...
			case 'MOWER':
				if (a === '.activity_value') {
					...
					if (mowerRunning === true) {
						valvesArr = getSavedServices('SERVICE_VALVE'); 

						// for all valves
						for (let i = 0; i < valvesArr.length; i++) {
							let v_i = valvesArr[i];
							// valve open?
							if (getSaveState(v_i + '.activity_value') !== 'CLOSED') {
								// allowed to be open?
								if (getSaveState(v_i + '.' + IRRWHILEMOWING_ALLOWED) === false) {
									// not allowed for this mower?
									mowerDefinitionString = getSaveState(v_i + '.' + IRRWHILEMOWING_MOWER_DEFINITION);
									warningcode = '';
...
function prepLeftoverTimer(activitytimer, name, a, type, propval) {
	...
	if (a === '.activity_value') {
		...
		if (propval !== 'TIME_LIMITED_ON') {
			...
			nx1 = name + x1;
			//setGardenaObjectNotExists(nx1, defnull);
			adapter.setObjectNotExists(sgEncode(nx1), {
				type: 'state',
				common: {
					name: x1.substr(1),
	...
	if (activitytimer.valid === true && activitytimer.duration !== 'null' && activitytimer.timestamp !== 'null') {
		...
		//setGardenaObjectNotExists(nx, difference);
...
function setForecastState(name, a, type, propval) {
	...
	//setGardenaObjectNotExists(nx, propval);
	adapter.setObjectNotExists(sgEncode(nx), {
		type: 'state',
		common: {
			name: a.substr(1),
...
function checkAndSetForecast(name, a, type, propval, m) {
	...
	if (type === 'COMMON' && a === '.batteryLevel_value') {
		...
		// charging or mowing? 
		oldval = getSaveState(name + '.batteryState_value');
		if (oldval === 'CHARGING') {
					...
					if (propattributes === 'attributes') {
						for (let prop2 in m[propattributes]) {
							for (let prop3 in m[propattributes][prop2]) {
								...
								//setGardenaObjectNotExists(n, ma_val);
		...
		if (dp.hasOwnProperty(service)) {
			for (name in dp[service]) {
				if (dp[service][name].predefineForDevices.indexOf(maindevicetype) >= 0) {
					setGardenaObjectNotExists(id + '.' + name, '');
...
function parseMessage(msg) {
	...
	switch (m.type) {
		case "LOCATION":
			...
			//setGardenaObjectNotExists('LOCATION_' + m.id + '.name', m.attributes.name);
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 2135 $
 */
