 ...
 * SVN:                 $Rev: 2134 $ ($Date: 2020-06-04 22:27:44 +0200 (Do, 04 Jun 2020) $)
 * contains some functions available at forum.iobroker.net, see function header
 */
"use strict";

// some variables 
const apirev = '$Rev: 2134 $';
...
function getSaveStateNameWildcard(s1, wildcard, s2) {
	...
	ju.adapterloginfo(3, 'getSaveStateNameWildcard: s=' + s );

	for (let id in internSaveState) {
		if (matchRuleShort(id, s) === true) {
			s = id.substr(0, id.length - '.activity_value'.length);	
			ju.adapterloginfo(3, 'getSaveStateNameWildcard: id=' + adapter_instance + s + s2 );
			return adapter_instance + s + s2;
			break;
		}
	}
	ju.adapterloginfo(3, 'getSaveStateNameWildcard: undefined');
	return UNDEFINED;
}
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 2134 $
 */
